# 🏆 CRYPTx-CTF-2026

Welcome to the official repository for **Team GHOST-X**. This project documents our technical journey through the **CryptX CTF 2026**, where we applied advanced forensics, reverse engineering, and OSINT techniques to solve complex security challenges.

## 👥 Meet the Team: GHOST-X

We are a 4-member squad of security analysts and developers. Connect with us to discuss CTFs, cybersecurity, or collaboration:

| Member | GitHub | LinkedIn |
| :--- | :--- | :--- |
| **Sajad Niflar** | [💻 Profile](https://github.com/Sajad-Niflar) | [🔗 Connect](https://www.linkedin.com/in/sajadniflar) |
| **Hafeez Saleem** | [💻 Profile](https://github.com/Haf-eez) | [🔗 Connect](https://www.linkedin.com/in/hafeez-saleem) |
| **Chithma Pathirana** | [💻 Profile](https://github.com/ChithMa) | [🔗 Connect](https://www.linkedin.com/in/chithma-pathirana-32389b323) |
| **Raqeeb Ameen** | [💻 Profile](https://github.com/rockeeebb) | [🔗 Connect](https://www.linkedin.com/in/rockeebb) |

-----

## 📊 Performance Summary

| Category | Stats |
| :--- | :--- |
| **Round 1 Status** | ✅ Completed |
| **Challenges Solved** | 9 / 10 |
| **Success Rate** | 90% |

-----

## 📂 Rounds & Writeups

### [🏁 First Round](https://github.com/Haf-eez/CTF-Writeups/tree/main/CRYPTx-CTF-2026/First-Round)

Our solutions for the opening round, including:

  * **Network Forensics:** ICMP packet analysis and reconstruction.
  * **Mobile Security:** Reverse engineering Android APKs (`jadx`).
  * **Audio Steganography:** Phase inversion and spectrogram analysis.
  * **OSINT:** Tracking digital footprints across GitHub and the Wayback Machine.
